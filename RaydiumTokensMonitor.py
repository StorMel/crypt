...
class RaydiumTokensMonitor(threading.Thread):
    ...
    async def monitor_token(self, token_address: str):
        if self.wsocket:
            if token_address in self.token_infos:
                ...
                token_info = TokensApi.get_amm_token_pool_data(token_address,"EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v")
    ...
    def _update_price(self, token_address: str):
        if token_address in self.token_infos:
            ...
            if sol_balance and token_info.token_vault_ui_amount > 0:
                sol_balance /= 1e9 #lamports translation

                token_info.price = sol_balance / token_info.token_vault_ui_amount

    async def _read_socket(self):#1:51:24 in the vid - web socket and rpc
       # while True:
            try:
    ...
    def _process(self, data: dict):
        ...
        if params:
            ...
            #pubsub.publish(topicName=globals.topic_token_update_event, arg1=token_address)
            print(str(token_address) + " has been updated")
